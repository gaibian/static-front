require('../../less/mp.less');

let mp = {
  //普通弹出提示层
  toast:(message)=>{
    "use strict";
    let toastEl = $('.toast_popup');
    if(toastEl && toastEl.length > 0){
      toastEl.show();
      toastEl.find('.text').text(message);
    }else{
      let toastDom = $(`<div class="toast_popup"><div class="toast_content"><p class="text">${message}</p></div></div>`);
