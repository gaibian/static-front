require('../../less/verify_error.less');
const verify=(function(){
  ...
  class bgBind extends validation{
        ...
        $this.on('blur',()=>{
          ...
          for(let i=0;i<that.flagArr.length;i++){
            if(that.flagArr[i]['check'] == 'false'){
              ...
              if(that.obj.throughState){
                that.obj.throughState(that.flag);
              }
              break;
            }else{
          ...
          if(that.flagArr[i]['check'] == 'false'){
            ...
            if(typeof dataNullError != 'undefined' && dataNull == 'true'){  //输入的值为空
              ...
              if(typeof dataError === 'undefined'){
                ...
                that.error(errorVal,that.flagArr[i]['el']);
              }else{
                that.error(dataError,that.flagArr[i]['el']);
              }
            }
            that.flag = false;  //数据没有通过
            break;
    ...
    checkIsOK(This,type,index){ //对每个input进行验证
      ...
      let dataError = This.attr('data-error');
      let dataNull = This.attr('data-empty');
      let dataNullError = This.attr('data-null-error');
      if(typeof obj['rule'] === 'function'){
        ...
        if($.trim(text) === ''){  //为空
          ...
          This.attr('data-empty',true);
          if(typeof dataNullError != 'undefined' && dataNull == 'true'){  //输入的值为空
            this.error(dataNullError,This);
          }else{
            if(typeof dataError === 'undefined'){
              let errorVal = obj['error'];
              this.error(errorVal,This);
            }else{
              this.error(dataError,This);
            }
          }
          //this.error(,This);
        }else{
    ...
    error(message,This){
      let that = this;
      if(this.errorPopup && this.errorPopup.length > 0){
        this.errorPopup.remove();
      }
      let errorDom = $(`<div class="error_message">
          <span class="message">${message}</span>
          <i class="s_bg"></i>
          </div>`);
      This.parent().append(errorDom);
      this.errorPopup = $('.error_message');
      let _height = this.errorPopup.outerHeight(true);
      this.errorPopup.css({
        bottom:-_height + 'px'
      });
      setTimeout(function(){
