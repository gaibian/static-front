require('../../less/pic_upload.less');
const picUpLoad = (()=>{
  "use strict";
  let _queue = [];
  class upload{
    constructor(dom,index,url,callback){
      this.dom = dom;
      this.filechooser = dom.querySelector('input[type=file]');
      //用于压缩图片的canvas
      ...
      xhr.onreadystatechange = ()=>{
        ...
        if(xhr.readyState == 4 && xhr.status == 200){
          ...
          if(this.stateArr.length === length){
            this.callback(this.stateArr,this.dom,this.index);
            let message = '上传成功';
  ...
  return (opt)=>{
    let wrapper = typeof opt.el === 'string' ? document.querySelectorAll(opt.el) : opt.el;
    for(let i=0;i<wrapper.length;i++){
      let $this = wrapper[i];
      if($this.className.indexOf('init') === -1){
        _queue.push(new upload($this,i,opt.url,opt.callback));
        $this.className += ' init';
      }
    }
  }
