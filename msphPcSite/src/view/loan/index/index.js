...
$(()=>{
  ...
  /*贷款计算弹窗*/
  const popupCalcBtn = $('.popup_calc_btn');
  const popupCalcDom = $('.loan_calc_popup');
  const popupCloseBtn = popupCalcDom.find('.close_btn');
  popupCalcBtn.on('click',()=>{
    popupCalcDom.show();
  });
  ...
  const calc = (()=>{
    ...
    class calcFunB{
      constructor(val1,val2,val3){
        ...
        console.log(this.interMonthInputVal);
      }
      getCalc(){
        ...
        console.log(this.eachRepaymentVal);
