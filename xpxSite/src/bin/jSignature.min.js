...
(function () {
  function t(b) {
    ...
    b = /rgb[a]*\((\d+),\s*(\d+),\s*(\d+)/;
    var h = /#([AaBbCcDdEeFf\d]{2})([AaBbCcDdEeFf\d]{2})([AaBbCcDdEeFf\d]{2})/, m;
    a = void 0;
    (a = c.match(b)) ? m = {
      r: parseInt(a[1], 10),
      g: parseInt(a[2], 10),
      b: parseInt(a[3], 10)
    } : (a = c.match(h)) && (m = {r: parseInt(a[1], 16), g: parseInt(a[2], 16), b: parseInt(a[3], 16)});
    var d;
    e ? (a = void 0, (a = e.match(b)) ? d = {
      ...
      g: parseInt(a[2], 16),
      b: parseInt(a[3], 16)
    })) : d = m ? 127 < Math.max.apply(null, [m.r, m.g, m.b]) ? {r: 0, g: 0, b: 0} : {r: 255, g: 255, b: 255} : {
      r: 255,
      g: 255,
      b: 255
    };
    ...
    (b = -1), m = Math.round(m + 96 * b), m = {r: m, g: m, b: m}) : m = {r: 191, g: 191, b: 191};
  ...
  function k(b, a) {
    ...
    this.resizeTo = function (a) {
      if (0 === this.x && 0 === this.y) this._length = 0; else if (0 ===
          this.x) this._length = a, this.y = a * c(this.y); else if (0 === this.y) this._length = a, this.x = a * c(this.x); else {
        var b = Math.abs(this.y / this.x), e = Math.sqrt(Math.pow(a, 2) / (1 + Math.pow(b, 2))), b = b * e;
        this._length = a;
        this.x = e * c(this.x);
        this.y = b * c(this.y)
      }
  ...
  function n(b, a, c, e, h) {
    this.data = b;
    this.context = a;
    if (b.length) for (var m = b.length, d, l, f = 0; f < m; f++) {
      d = b[f];
      l = d.x.length;
      c.call(a, d);
    ...
    this.startStroke = function (a) {
      if (a && "number" == typeof a.x && "number" == typeof a.y) {
        ...
        this._lastPoint = a;
        this.inStroke = !0;
        var b = this._stroke, c = this.startStrokeFn, e = this.context;
        setTimeout(function () {
          c.call(e, b)
        }, 3);
        return a
    ...
    this.addToStroke = function (a) {
      if (this.inStroke && "number" === typeof a.x && "number" === typeof a.y && 4 < Math.abs(a.x - this._lastPoint.x) + Math.abs(a.y - this._lastPoint.y)) {
        ...
        this._lastPoint = a;
        var c = this._stroke, e = this.addToStrokeFn, h = this.context;
        setTimeout(function () {
          e.call(h, c, b)
        }, 3);
        return a
    ...
    this.endStroke = function () {
      ...
      if (a) {
        var b = this._stroke, c = this.endStrokeFn, e = this.context, h = this.changed;
        setTimeout(function () {
          c.call(e, b);
          h.call(e)
        }, 3);
    ...
    ".parentresized"] = e.subscribe(c + ".parentresized", function (a, m, d, l) {
      return function () {
        ...
        if (h !== d) {
          ...
          l = f.data;
          var h = 1 * h / d, u = [], A, E, g, k, r, n;
          E = 0;
          for (g = l.length; E < g; E++) {
            n = l[E];
            A = {x: [], y: []};
            k = 0;
            for (r = n.x.length; k < r; k++) A.x.push(n.x[k] * h), A.y.push(n.y[k] * h);
            u.push(A)
          }
          f.data = u;
          m[c](f)
  ...
  function x(b, a, c) {
    ...
    var h = $.fn.jSignature("globalEvents"), d = {
      width: "ratio",
      height: "ratio",
      sizeRatio: 4,
      color: "#000",
    ...
    this.$controlbarUpper = $('<div style="padding:0 !important; margin:0 !important;width: 100% !important; height: 0 !important; -ms-touch-action: none; touch-action: none;margin-top:-1em !important; margin-bottom:1em !important;"></div>').appendTo(e);
    this.isCanvasEmulator = !1;
    a = this.canvas = this.initializeCanvas(d);
    c = $(a);
    this.$controlbarLower = $('<div style="padding:0 !important; margin:0 !important;width: 100% !important; height: 0 !important; -ms-touch-action: none; touch-action: none;margin-top:-1.5em !important; margin-bottom:1.5em !important; position: relative;"></div>').appendTo(e);
  ...
  function y(b) {
    ...
    if (c) {
      ...
      if (1 !== c) try {
        $(b).children("object").get(0).resize(Math.ceil(b.width * c), Math.ceil(b.height * c)), b.getContext("2d").scale(c, c)
      } catch (e) {
      }
      return !0
    }
  ...
  var w = function (b, a) {
    ...
    this.publish = function (a, b, e, d) {
      if (this.topics[a]) {
        var c = this.topics[a], h = Array.prototype.slice.call(arguments, 1), f = [], g = [], p, q, u, A;
        q = 0;
        for (u = c.length; q < u; q++) A = c[q], p = A[0], A[1] && (A[0] = function () {
        }, f.push(q)), g.push(p);
        q = 0;
    ...
    var a = this.canvasContext, c = b.x[0];
    b = b.y[0];
    var e = this.settings.lineWidth, d = a.fillStyle;
    a.fillStyle = a.strokeStyle;
    a.fillRect(c +
        e / -2, b + e / -2, e, e);
    a.fillStyle = d
  }, f = function (b, a) {
    var c = new g(b.x[a - 1], b.y[a - 1]), e = new g(b.x[a], b.y[a]), d = c.getVectorToPoint(e);
    if (1 < a) {
      var m = new g(b.x[a - 2], b.y[a - 2]), f = m.getVectorToPoint(c), l;
      if (f.getLength() > this.lineCurveThreshold) {
        l = 2 < a ? (new g(b.x[a - 3], b.y[a - 3])).getVectorToPoint(m) : new k(0, 0);
        var n = .35 * f.getLength(), p = f.angleTo(l.reverse()), q = d.angleTo(f.reverse());
        l = (new k(l.x + f.x, l.y + f.y)).resizeTo(Math.max(.05, p) * n);
        var u = (new k(f.x + d.x, f.y + d.y)).reverse().resizeTo(Math.max(.05, q) * n), f = this.canvasContext,
            n = m.x, q = m.y, p = c.x, A = c.y, r = m.x + l.x, m = m.y + l.y;
        l = c.x + u.x;
        u = c.y + u.y;
        f.beginPath();
        f.moveTo(n, q);
    ...
    var a = b.x.length - 1;
    if (0 < a) {
      var c = new g(b.x[a], b.y[a]), e = new g(b.x[a - 1], b.y[a - 1]), d = e.getVectorToPoint(c);
      if (d.getLength() > this.lineCurveThreshold) if (1 < a) {
        b = (new g(b.x[a - 2], b.y[a - 2])).getVectorToPoint(e);
        var f = (new k(b.x + d.x, b.y + d.y)).resizeTo(d.getLength() / 2), d =
            this.canvasContext;
        b = e.x;
        var a = e.y, E = c.x, l = c.y, n = e.x + f.x, e = e.y + f.y, f = c.x, c = c.y;
        d.beginPath();
        d.moveTo(b, a);
  ...
  x.prototype.resetCanvas = function (b, a) {
    var c = this.canvas, e = this.settings, h = this.canvasContext, m = this.isCanvasEmulator, g = c.width,
        l = c.height;
  ...
  x.prototype.initializeCanvas = function (b) {
    ...
    "ratio" === b.height ? c.css("height", Math.round(c.width() /
        b.sizeRatio)) : "ratio" === b.width && c.css("width", Math.round(c.height() * b.sizeRatio));
  ...
  (function (b) {
    function a(a, b, c) {
      ...
      d.onload = function () {
        var a = e.getContext("2d"), b = a.shadowColor;
    ...
    function d(a, b) {
      ...
      }(c.data("jSignature.this"))); else throw Error("jSignature is unable to find import plugin with for format '" + String(b) + "'");
      return this
    }
    ...
    var f = {}, g = {
      ...
      "native": function (a, b, c) {
        c(a)
      }, image: a, "image/png;base64": a, "image/jpeg;base64": a, "image/jpg;base64": a
    }, k = function (a) {
      var b = !1;
      for (a = a.parentNode; a && !b;) b = a.body, a = a.parentNode;
      return !b
    }, p = {"export": g, "import": l, instance: f}, n = {
      init: function (a) {
        ...
        return d.settings[a]
      }, clear: c, reset: c, addPlugin: function (a, b, c) {
      ...
      importData: d, setData: d, globalEvents: function () {
        return h
      }, disable: function () {
...
(function () {
  for (var t = {}, k = {}, g = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWX".split(""), n = g.length / 2, r = n - 1; -1 < r; r--) t[g[r]] = g[r + n], k[g[r + n]] = g[r];
  var x = function (d) {
    d = d.split("");
    for (var b = d.length, a = 1; a < b; a++) d[a] = t[d[a]];
    return d.join("")
  }, y = function (d) {
    for (var b = [], a = 0, c = 1, e = d.length, h, f, g = 0; g < e; g++) h = Math.round(d[g]), f = h - a, a = h, 0 > f && 0 < c ? (c = -1, b.push("Z")) : 0 < f && 0 > c && (c = 1, b.push("Y")), h = Math.abs(f), h >= n ? b.push(x(h.toString(n))) : b.push(h.toString(n));
    return b.join("")
  }, w = function (d) {
    var b =
        [];
    d = d.split("");
    for (var a = d.length, c, e = 1, f = [], m = 0, g = 0; g < a; g++) c = d[g], c in t || "Z" === c || "Y" === c ? (0 !== f.length && (f = parseInt(f.join(""), n) * e + m, b.push(f), m = f), "Z" === c ? (e = -1, f = []) : "Y" === c ? (e = 1, f = []) : f = [c]) : f.push(k[c]);
    ...
    var b = [];
    d = d.split("_");
    for (var a = d.length / 2, c = 0; c < a; c++) b.push({x: w(d[2 * c]), y: w(d[2 * c + 1])});
    return b
  }, D = function (d) {
...
(function () {
  ...
  function n(f, d, b) {
    d += 1;
    var a = new k(f.x[d - 1], f.y[d - 1]), c = new k(f.x[d], f.y[d]), c = a.getVectorToPoint(c),
        e = new k(f.x[d - 2], f.y[d - 2]), a = e.getVectorToPoint(a);
    return a.getLength() > b ? (b = 2 < d ? (new k(f.x[d - 3], f.y[d - 3])).getVectorToPoint(e) : new t(0, 0), f = .35 * a.getLength(), e = a.angleTo(b.reverse()), d = c.angleTo(a.reverse()), b = (new t(b.x + a.x, b.y + a.y)).resizeTo(Math.max(.05,
        e) * f), c = (new t(a.x + c.x, a.y + c.y)).reverse().resizeTo(Math.max(.05, d) * f), c = new t(a.x + c.x, a.y + c.y), ["c", g(b.x, 2), g(b.y, 2), g(c.x, 2), g(c.y, 2), g(a.x, 2), g(a.y, 2)]) : ["l", g(a.x, 2), g(a.y, 2)]
  }

  function r(f, d) {
    var b = f.x.length - 1, a = new k(f.x[b], f.y[b]), c = new k(f.x[b - 1], f.y[b - 1]), a = c.getVectorToPoint(a);
    if (1 < b && a.getLength() > d) {
      var b = (new k(f.x[b - 2], f.y[b - 2])).getVectorToPoint(c), c = a.angleTo(b.reverse()), e = .35 * a.getLength(),
  ...
  function w(f, d) {
    var b = ['<?xml version="1.0" encoding="UTF-8" standalone="no"?>', '<!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.1//EN" "http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd">'],
        a, c = f.length, e, h = [], g = [], k = e = a = 0, l = 0, n = [];
    if (0 !== c) {
      for (a = 0; a < c; a++) {
        k = f[a];
        l = [];
  ...
  (function (f, d) {
    ...
    (typeof exports != d + "" ? exports : f).simplify = function (b, a, c) {
      a = a !== d ? a * a : 1;
      if (!c) {
        var e = b.length, f, g = b[0], k = [g];
        for (c = 1; c < e; c++) {
          f = b[c];
          var l = f.x - g.x, n = f.y - g.y;
      ...
      f = b;
      c = f.length;
      var e = new (typeof Uint8Array != d + "" ? Uint8Array : Array)(c), g = 0, k = c - 1, p, q, r = [], t = [], z = [];
      for (e[g] = e[k] = 1; k;) {
        n = 0;
        for (l = g + 1; l < k; l++) {
          p = f[l];
          var B = f[g], x = f[k], v = B.x, w = B.y, B = x.x - v, C = x.y - w, y;
          if (0 !== B || 0 !== C) y = ((p.x - v) * B + (p.y - w) * C) / (B * B + C * C), 1 < y ? (v = x.x, w = x.y) : 0 < y && (v += B * y, w += C * y);
          p = (B = p.x -
              v, C = p.y - w, B * B + C * C);
          p > n && (q = l, n = p)
        }
  ...
  if ("function" !== typeof D) var D = function (f) {
    var d = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".split(""), b, a, c, e, g = 0, k = 0,
        n = [];
    do b = f.charCodeAt(g++), a = f.charCodeAt(g++), c = f.charCodeAt(g++), e = b << 16 | a << 8 | c, b = e >> 18 & 63, a = e >> 12 & 63, c = e >> 6 & 63, e &= 63, n[k++] = d[b] + d[a] + d[c] + d[e]; while (g < f.length);
    d = n.join("");
    f = f.length % 3;
