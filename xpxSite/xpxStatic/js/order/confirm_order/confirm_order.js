...
/******/ 	return hotCreateRequire(125)(__webpack_require__.s = 125);
/******/ })
/************************************************************************/
/******/ ([
/* 0 */
...
var mp = {
  ...
  toast: function toast(message) {
    ...
    var toastEl = $('.toast_popup');
    if (toastEl && toastEl.length > 0) {
      toastEl.show();
      toastEl.find('.text').text(message);
    } else {
      var toastDom = $('<div class="toast_popup"><div class="toast_content"><p class="text">' + message + '</p></div></div>');
  ...
  verify: function () {
    ...
    var validation = function () {
      ...
      validation.prototype.init = function init() {
        //初始化
    ...
    var bgBind = function (_validation) {
      (0, _inherits3.default)(bgBind, _validation);

      function bgBind(obj) {
        (0, _classCallCheck3.default)(this, bgBind);

        var _this = (0, _possibleConstructorReturn3.default)(this, _validation.call(this, obj));

        _validation.prototype.init.call(_this);
        _this.init();
        _this.bind();
      ...
      bgBind.prototype.init = function init() {
        var that = this;
        this.input.each(function (index) {
          var $this = that.input.eq(index);
          var val = $this.val();
          if (val === '') {
      ...
      bgBind.prototype.bind = function bind() {
        var that = this;
        this.sendBtn.on('click', function () {
          ...
          for (var i = 0; i < that.flagArr.length; i++) {
            if (that.flagArr[i]['check'] == 'false') {
              var dataError = that.flagArr[i]['el'].attr('data-error');
              if (typeof dataError === 'undefined') {
                var errorVal = that.rules[that.flagArr[i]['type']]['error'];
                that.error(errorVal);
              } else {
      ...
      bgBind.prototype.checkIsOK = function checkIsOK(This, type, index) {
        //对每个input进行验证
        var text = This.val();
        var obj = this.rules[type];
        if (typeof obj['rule'] === 'function') {} else {
      ...
      //验证出错
      bgBind.prototype.error = function error(message) {
        var that = this;
        ...
        if (this.errorPopup.length > 0) {
          ...
          var errorDom = $('<div class="error_popup"><div class="error_content"><div class="img"></div><p class="text">' + message + '</p></div></div>');
      ...
      bgBind.prototype.success = function success() {
        //console.log('单独验证通过');
      };
...
window.mp = __webpack_require__(83);
$(function () {
  "use strict";
  ...
  var status = function () {
    ...
    var statusFun = function () {
      function statusFun(el) {
        (0, _classCallCheck3.default)(this, statusFun);
      ...
      statusFun.prototype.init = function init() {
        this.bind();
      };

      statusFun.prototype.bind = function bind() {
        var _this = this;

        this.statusBox.each(function (index) {
          _this.statusBox.eq(index).on('click', function () {
            var statusBg = _this.statusBox.eq(index).find('.status_bg');
            var input = _this.statusBox.eq(index).find('input');
            if (statusBg.is(":hidden")) {
              _this.el.find('.status_bg').hide();
              statusBg.show();
              _this.el.find('input[type=checkbox]').each(function (index) {
                _this.el.find('input[type=checkbox]').eq(index).prop('checked', false);
              });
    ...
    return function (el) {
      var wrapper = typeof el === 'string' ? $(el) : el;
      wrapper.each(function (index) {
        var $this = wrapper.eq(index);
  ...
  var timePick = $('#time_picker');
  var timeVal = timePick.find('.time_val');
  timePick.on('click', function () {
    ...
    dtPicker.show(function (e) {
      var y = Number(e.y.value) + 1;
      var m = e.m.value;
      var d = e.d.value;
      var val = e.value + '\u81F3' + y + '-' + m + '-' + d;
      timeVal.text(val);
